...
public class ElementCore : PropertyCore
{
    ...
    protected bool targetIsKilled;
    // 每次受到攻击时更新该变量，表示攻击来源
    protected BattleCore lastAttacker;
    ...
    // 受到元素伤害改变委托列表，由其他类注册，前委托的输出会作为后委托的输入
    ...
    public bool IsAttachElement(ElementType elementType)
    {
        return attachedElement.Any(i => i.Key == elementType && i.Value > 0);
    }
        ...
        bool reversal = false)
    {
        ...
        {// 复制一份委托并遍历
            List<Func<ElementSlot, float, float>> tmp = 
                new List<Func<ElementSlot, float, float>>(getElementDamFuncList);
