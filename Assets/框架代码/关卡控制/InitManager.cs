...
public class InitManager : MonoBehaviour
{
    ...
    public static bool enemyWaveControllerIsNull = true;
    ...
    public static OperatorCore GetNearestOper(Vector3 center, float r)
    {// 在范围内找到距离center最近的oper
        List<OperatorCore> tarList = GetNearByOper(center, r);
        OperatorCore tar = null;
        float minDis = 1e9f;
        foreach (var oc_ in tarList)
        {
            float dis = BaseFunc.xz_Distance(center, oc_.transform.position);
            if (dis < minDis)
            {
                minDis = dis;
                tar = oc_;
    ...
    public static EnemyCore GetNearestEnemy(Vector3 center, float r)
    {// 在范围内找到距离center最近的enemy
        List<EnemyCore> tarList = GetNearByEnemy(center, r);
        EnemyCore tar = null;
        float minDis = 1e9f;
        foreach (var ec_ in tarList)
        {
            float dis = BaseFunc.xz_Distance(center, ec_.transform.position);
            if (dis < minDis)
            {
                minDis = dis;
                tar = ec_;
