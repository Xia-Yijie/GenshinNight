...
public class DragSlot : MonoBehaviour,IBeginDragHandler,IDragHandler,IEndDragHandler
{
    ...
    private Vector3 preScale;               // 原始anim缩放
    ...
    private void Awake()
    {
        ...
        DeployAudio = transform.parent.GetComponent<AudioSource>();
    }
    ...
    public void Refresh(OperatorCore oc_)
    {
        ...
        float costVal = operatorCore.costNeed.val;
        if (costVal < operatorCore.costNeed.baseVal) costVal -= 1e-6f;
        else costVal += 1e-6f;
        costText.text = costVal.ToString("f0");
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        preScale = operatorAnim.localScale;
    ...
    {
        operatorAnim.localScale = preScale;
    }

    public void AnimTurnLeft()
    {
        Vector3 scale = new Vector3(-preScale.x, preScale.y, preScale.z);
        operatorAnim.localScale = scale;
    }
    ...
    public void PutSuccessfully()
    {
        ...
        // 播放部署语音
        DeployAudio.Play();
        if (operatorCore.od_.Deploy.Count != 0)
        {
            int id = Random.Range(0, operatorCore.od_.Deploy.Count);
            AudioManager.OperatorTalk(operatorCore.od_.Deploy[id]);
        }
