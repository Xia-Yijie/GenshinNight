...
public class OperUIFunc : MonoBehaviour
{
    ...
    public void Skill()
    {
        ...
        // 如果技能正在持续中，判断能否提前结束，如果可以结束，则结束技能
        if (sp_.during)
        {
    ...
    public void Elitism()
    {
        ...
        {
            AudioManager.OperatorTalk(od_.ElitismUP[oc_.eliteLevel]);
        }
    ...
    public void SkillLevelUp()
    {
        ...
        // 根据选择的技能重置spController,保留当前sp
        int lel = oc_.skillLevel[oc_.skillNum];
        switch (oc_.skillNum)
        {
            case 0:
                oc_.sp_.Init(oc_, oc_.sp_.sp, od_.maxSP0[lel], od_.duration0[lel],
                    od_.skill0_recoverType, od_.skill0_releaseType, od_.spRecharge);
                break;
            case 1:
                oc_.sp_.Init(oc_, oc_.sp_.sp, od_.maxSP1[lel], od_.duration1[lel],
                    od_.skill1_recoverType, od_.skill1_releaseType, od_.spRecharge);
                break;
            case 2:
                oc_.sp_.Init(oc_, oc_.sp_.sp, od_.maxSP2[lel], od_.duration2[lel],
                    od_.skill2_recoverType, od_.skill2_releaseType, od_.spRecharge);
                break;
        }
        OperUIManager.skillUIController.Refresh();
