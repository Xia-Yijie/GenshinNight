...
public class Steward : OperatorCore
{
    ...
    public override string GetSkillDescription(int SkillID)
    {
        ...
        {
            return "下次攻击的攻击力提高至" +
                   CT.ChangeToColorfulPercentage(Skill_1_Multi[skillLevel[0]], CT.normalBlue);
        }
        else
        {
            return "史都华德凝聚记忆中的风雪，在目标脚下召唤一片冰雾区。冰雾会持续对其中的敌人造成" +
                   CT.GetColorfulText("冰元素魔法", CT.CryoWhite) +
                   "伤害，每次伤害的倍率为" +
                   CT.ChangeToColorfulPercentage(FogMulti[skillLevel[1]], CT.normalBlue) +
