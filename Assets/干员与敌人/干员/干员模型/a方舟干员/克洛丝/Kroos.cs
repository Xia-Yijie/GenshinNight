...
public class Kroos : OperatorCore
{
    ...
    private void Archery(float multi, GameObject proArrow, 
        Action<float, BattleCore, parabola, bool> endAttack)
    {// 射一支箭出去，攻击倍率为multi
    ...
    private void PyroAttack(float multi, BattleCore tarBC, parabola par, bool tarIsNull)
    {
        if (tarIsNull) return;
    ...
    public override string GetSkillDescription(int SkillID)
    {
        ...
        if (SkillID == 0)
        {
            return "克洛丝的下次攻击会连续射击2次，每次射击造成攻击力" +
                   CT.ChangeToColorfulPercentage(skill1Atk[lel], CT.normalBlue)
                   + "的普通物理伤害";
        }
        else
        {
            return "克洛丝的下次攻击会发射一支充满火元素的箭矢，造成攻击力" +
                   CT.ChangeToColorfulPercentage(skill2Atk[lel], CT.normalBlue)
                   + "的" +
    ...
    public override string GetTalentDescription(int talentID)
    {
        ...
        {
            return "攻击时，" +
                   CT.ChangeToPercentage(talent1Probability[eliteLevel]) +
                   "几率当次攻击的攻击力提升至150%";
        }
        else
        {
