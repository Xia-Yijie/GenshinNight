...
public class Melantha : OperatorCore
{
    ...
    public override string GetSkillDescription(int SkillID)
    {
        ...
        {
            return "攻击力+" +
                   CT.ChangeToColorfulPercentage(AtkIncrease[skillLevel[0]], CT.normalBlue);
        }
        else
        {
            return "玫兰莎汇聚雷电的力量，快速拔刀向前方挥出多剑，造成3次" +
                   CT.GetColorfulText("雷元素物理", CT.ElectroPurple) +
                   "范围伤害，每次伤害的倍率为" +
                   CT.ChangeToColorfulPercentage(elecAtkMulti[skillLevel[1]], CT.normalBlue) +
                   "攻击力\n" +
