...
public class Tartaglia : OperatorCore
{
    ...
    public AudioSource eStartAudio;
    public AudioSource eAtkAudio;
    public AudioSource hydroBoomAudio;
    ...
    private float riptideDuring_Burst = 2f;
    ...
    private float riptideBurst_Multi = 0.6f;
    ...
    {// 有probability的概率给目标上断流
        ...
        TartagliaRiptideSlot slot = new TartagliaRiptideSlot(riptideObj, t);
    ...
    public void Skill2CauseDamage()
    {
        ...
        foreach (var ec_ in tars)
        {
            ...
            // 断流-爆
            if (!RiptideDic.ContainsKey(ec_)) GiveRiptide(ec_, riptideProbability_2, riptideDuring);
            else
            {
                RiptideDic.Remove(ec_);
                multi = 1.5f;
    ...
    public override void SkillAtk_3()
    {
        ...
        foreach (var bc_ in enemyList)
        {
            ...
            Vector3 pos = ec_.animTransform.localPosition + new Vector3(0, 0, 0.3f);
            ...
            GiveRiptide(ec_, riptideProbability_3, riptideDuring);
        }
    }
    ...
    private void hydroAttack(float multi, BattleCore tarBC, parabola par, bool tarIsNull)
    {// 一技能的水箭，射到了敌人身上
        if (tarIsNull) return;
        
        GameObject hitAnim = PoolManager.GetObj(aimHitAnim);
        hitAnim.transform.SetParent(tarBC.frontCanvas.transform);
        Vector3 pos = tarBC.animTransform.position;
        pos.z += 0.3f;
        ...
        if(RiptideDic.ContainsKey(tarEC))
        {
            ...
            center.y = 0;
            riptideFlash.transform.position = tarEC.transform.position;
        ...
        GiveRiptide(tarEC, riptideProbability_1, riptideDuring);
    }
    ...
    private void RiptideBurstWhenDie(BattleCore bc_)
    {
        ...
        foreach (var tarEC in tars)
        {
            GiveRiptide(tarEC, riptideProbability_1, riptideDuring_Burst);
    ...
    {
        if (skillNum != 2) return;

        sp_.sp = sp_.remainingTime > sp_.maxSp ? sp_.maxSp : sp_.remainingTime;
    }
        ...
        switch (SkillID)
        {
            case 0:
                return "达达利亚的下一次攻击将造成" +
                       ...
                       "伤害\n\n本次攻击" +
                       CT.GetColorfulText("必定", CT.normalBlue) +
                       "给敌人附加断流效果\n\n若本次攻击命中处于断流状态下的敌人，则会触发" +
                       CT.GetColorfulText("断流闪", CT.normalBlue) +
                       "\n" +
                       CT.GetColorfulText("断流闪", CT.normalBlue) +
                       "：在目标周围连续造成3次" +
                       CT.ChangeToColorfulPercentage(riptideFlash_Multi[lel]) +
                       "攻击力的" +
                       CT.GetColorfulText("水元素物理", CT.HydroBlue) +
                       "伤害";
            case 1:
                return "达达利亚拉起大弓，引导“魔弹一闪”攻击敌人\n攻击范围" +
                       ...
                       "，攻击间隔" +
                       CT.GetColorfulText("延长", CT.normalRed) +
                       "，每次攻击造成" +
                       CT.ChangeToColorfulPercentage(skill2_Multi[lel]) +
                       "攻击力的范围" +
                       CT.GetColorfulText("水元素物理", CT.HydroBlue) +
                       "伤害\n\n断流的附着概率变为" +
                       CT.ChangeToColorfulPercentage(riptideProbability_2) +
                       "\n\n该状态下的攻击命中处于断流状态下的敌人时，会" +
                       CT.GetColorfulText("消耗", CT.normalRed) +
                       "断流效果，并触发" +
                       CT.GetColorfulText("断流爆", CT.normalBlue) +
                       "\n" +
                       CT.GetColorfulText("断流爆", CT.normalBlue) +
                       "：本次攻击造成的伤害提高" +
                       CT.GetColorfulText("50%", CT.normalBlue);
            default:
                return "解放纯水构成的武装，进入魔王武装形态\n攻击范围" +
                       CT.GetColorfulText("缩小", CT.normalRed) +
                       "，攻击间隔" +
                       CT.GetColorfulText("缩短") +
                       "，同时攻击范围内的所有敌人。每次攻击造成" +
                       CT.ChangeToColorfulPercentage(skill3_Multi[lel]) +
                       "攻击力的" +
                       CT.GetColorfulText("水元素物理", CT.HydroBlue) +
                       "伤害。可手动结束技能，结束技能时会根据剩余持续时间，返还一定比例的sp\n\n断流的附着概率变为" +
                       CT.ChangeToColorfulPercentage(riptideProbability_3) +
                       "\n\n该状态下的攻击命中处于断流状态下的敌人时，会触发" +
                       CT.GetColorfulText("断流斩", CT.normalBlue) +
                       "\n" +
                       CT.GetColorfulText("断流斩", CT.normalBlue) +
                       "：在目标周围造成" +
                       CT.ChangeToColorfulPercentage(riptideSlash_Multi[lel]) +
                       "攻击力的" +
                       CT.GetColorfulText("水元素物理", CT.HydroBlue) +
                       "伤害";
    ...
    public override string GetTalentDescription(int talentID)
    {
        if (talentID == 1)
        {
            ...
            {
                case 0:
                    percentage = CT.ChangeToColorfulPercentage(
                        riptideProbability_1, CT.normalBlue);
                    break;
                case 1:
                    percentage = CT.ChangeToColorfulPercentage(
                        riptideProbability_2, CT.normalBlue);
                    break;
                default:
                    percentage = CT.ChangeToColorfulPercentage(
                        riptideProbability_3, CT.normalBlue);
                    break;
            }
        ...
        else
        {
            ...
            return "若敌人在断流状态下死亡，则会触发" +
                   CT.GetColorfulText("断流破", CT.normalBlue) +
                   "\n" +
                   CT.GetColorfulText("断流破", CT.normalBlue) +
                   "：在目标周围造成" +
                   CT.ChangeToColorfulPercentage(riptideBurst_Multi) +
                   "攻击力的" +
                   CT.GetColorfulText("水元素物理", CT.HydroBlue) +
                   "伤害，并为命中的敌人施加断流效果\n通过这种方式施加的断流效果只能持续" +
...
public class TartagliaSkill2 : SkillBuffSlot
{
    ...
    public override void BuffStart()
    {
        ...
        SkillAtkSpeedBuff atkSpeedBuff = new SkillAtkSpeedBuff(
            tartaglia.atkSpeedController, 0, tartaglia, 3f);
...
public class TartagliaSkill3 : SkillBuffSlot
{
    ...
    public override void BuffStart()
    {
        ...
        tartaglia.ChangeAtkRange(tartaglia.Skill3_AtkRange, true);
        // float atkSpeedInc = tartaglia.skill3_atkSpeedIncrease[tartaglia.skillLevel[2]];
        SkillAtkSpeedBuff atkSpeedBuff = new SkillAtkSpeedBuff(
            tartaglia.atkSpeedController, 0, tartaglia, 0.94f);
        BuffManager.AddBuff(atkSpeedBuff);
    }
